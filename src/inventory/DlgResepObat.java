...
public final class DlgResepObat extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        DTPBeri.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "20-05-2019" }));
...
//                "where resep_obat.no_resep='"+NoResep.getText()+"' and aturan_pakai.aturan<>''",param);
    ...
    private void ppBilResepActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppBilResepActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            param.put("emailrs", var.getemailrs());
            param.put("logo", Sequel.cariGambar("select logo from setting"));
            Valid.MyReport("rptResepJalan.jrxml", "report", "::[ Kwitansi Resep Obat ]::", " "
                    + " SELECT a.tgl_perawatan, a.jam, a.no_rawat, a.no_resep, a.no_rkm_medis, a.nm_pasien, a.png_jawab, "
                    + " a.nm_dokter, a.nama_brng, a.jml, a.hrg_jual, a.embalase, a.tuslah, a.total, b.total AS Total_Semua, "
                    + " a.nm_bangsal AS Asal_apotek, a.status_lanjut, a.Ruangan "
                    + " FROM ((SELECT detail_pemberian_obat.tgl_perawatan, detail_pemberian_obat.jam, detail_pemberian_obat.no_rawat, "
                    + " resep_obat.no_resep, reg_periksa.no_rkm_medis, pasien.nm_pasien, penjab.png_jawab, "
                    + " dokter.nm_dokter, databarang.nama_brng, detail_pemberian_obat.jml, detail_pemberian_obat.biaya_obat AS hrg_jual, "
                    + " detail_pemberian_obat.embalase, detail_pemberian_obat.tuslah, detail_pemberian_obat.total, "
                    + " bangsal.nm_bangsal, reg_periksa.status_lanjut, IFNULL(IF(reg_periksa.status_lanjut = 'RANAP',b2.nm_bangsal,poliklinik.nm_poli),'Belum Dapat Kamar') Ruangan "
                    ...
                    + " INNER JOIN resep_obat ON resep_obat.no_rawat = reg_periksa.no_rawat and resep_obat.tgl_perawatan = detail_pemberian_obat.tgl_perawatan and resep_obat.jam = detail_pemberian_obat.jam "
                    + " INNER JOIN riwayat_obat_pasien ON riwayat_obat_pasien.kode_brng = databarang.kode_brng and riwayat_obat_pasien.tanggal= detail_pemberian_obat.tgl_perawatan and mid(riwayat_obat_pasien.jam,1,5) = mid(detail_pemberian_obat.jam,1,5) and riwayat_obat_pasien.status = 'Simpan' "
                    + " and detail_pemberian_obat.no_rawat = riwayat_obat_pasien.no_rawat "
                    + " INNER JOIN bangsal ON riwayat_obat_pasien.kd_bangsal = bangsal.kd_bangsal "
                    + " INNER JOIN dokter ON dokter.kd_dokter = resep_obat.kd_dokter "
                    + " LEFT JOIN kamar_inap ON kamar_inap.no_rawat = reg_periksa.no_rawat and kamar_inap.stts_pulang = '-' "
                    + " LEFT JOIN kamar ON kamar.kd_kamar = kamar_inap.kd_kamar "
                    + " LEFT JOIN bangsal b2 ON b2.kd_bangsal = kamar.kd_bangsal "
                    + " LEFT JOIN poliklinik ON poliklinik.kd_poli = reg_periksa.kd_poli "
                    + " WHERE resep_obat.no_resep = '" + NoResep.getText() + "') AS a "
                    + " LEFT JOIN (select sum(d.total) total,r.no_resep from detail_pemberian_obat d  "
                    + " inner join resep_obat r on r.no_rawat = d.no_rawat and d.tgl_perawatan = r.tgl_perawatan and d.jam = r.jam "
                    + " WHERE r.no_resep ='" + NoResep.getText() + "') AS b ON a.no_resep = b.no_resep)", param);
