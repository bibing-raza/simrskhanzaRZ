  ...
  if(tbRetur.getValueAt(tbRetur.getSelectedRow(),0).toString().trim().equals("")){
         ...
         try {
            ...
            while(rs.next()){                
                ...
                try {
                    ...
                    while(rs2.next()){
                        Trackobat.catatRiwayat2(rs2.getString("kode_brng"),0,rs2.getDouble("jml_retur"),"Retur Jual",var.getkode(),rs.getString("kd_bangsal"),"Hapus");
    ...
    if(tabMode.getRowCount()==0){
            ...
            Map<String, Object> param = new HashMap<>(); 
                ...
                param.put("emailrs",var.getemailrs()); 
                param.put("logo",Sequel.cariGambar("select logo from setting"));             
                Valid.MyReport("rptBilReturJual.jrxml","report","::[ Bil. Pembayaran Retur Obat ]::"," SELECT RJ.tgl_retur, DRT.no_retur_jual, B.nm_bangsal AS asal_apotek, DRT.nota_jual, "+
